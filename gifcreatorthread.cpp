...
GifCreatorThread::GifCreatorThread(QObject *parent) : QObject(parent)
{
    mGifRaw = new GitRaw();
    connect(&mGifRaw->mWorkThread, &QThread::finished, this, &GifCreatorThread::finished);
}

GifCreatorThread::~GifCreatorThread()
{
    delete  mGifRaw;
}

void GifCreatorThread::startThread()
{
    mGifRaw->mWorkThread.start();
}

bool GifCreatorThread::isRunning()
{
    return mGifRaw->mWorkThread.isRunning();
}

void GifCreatorThread::begin(const QString &file, int width, int height, int delay, Qt::ConnectionType type)
{
    mWidth  = width;
    mHeight = height;
    QMetaObject::invokeMethod(mGifRaw, "begin", type,
...
void GifCreatorThread::frame(const QImage &img, int delay, Qt::ConnectionType type)
{
    // gif.h 文件有描述目前只能是RGBA8888图片格式，并且alpha没有被使用
    QImage img32 = img.convertToFormat(QImage::Format_RGBA8888);

    QMetaObject::invokeMethod(mGifRaw, "frame", type,
                              QGenericReturnArgument(),
                              Q_ARG(QImage, img32),
                              Q_ARG(int, mWidth),
                              Q_ARG(int, mHeight),
...
void GifCreatorThread::end(Qt::ConnectionType type)
{
    QMetaObject::invokeMethod(mGifRaw, "end", type);
}
...
int GitRaw::GifIMax(int l, int r) { return l>r?l:r; }
int GitRaw::GifIMin(int l, int r) { return l<r?l:r; }
int GitRaw::GifIAbs(int i) { return i<0?-i:i; }

// walks the k-d tree to pick the palette entry for a desired color.
// Takes as in/out parameters the current best color and its error -
// only changes them if it finds a better color in its subtree.
// this is the major hotspot in the code at the moment.
void GitRaw::GifGetClosestPaletteColor(GifPalette* pPal, int r, int g, int b, int& bestInd, int& bestDiff, int treeRoot)
{
...
void GitRaw::GifSwapPixels(uint8_t* image, int pixA, int pixB)
{
...
int GitRaw::GifPartition(uint8_t* image, const int left, const int right, const int elt, int pivotIndex)
{
...
void GitRaw::GifPartitionByMedian(uint8_t* image, int left, int right, int com, int neededCenter)
{
    if(left < right-1)
    {
...
void GitRaw::GifSplitPalette(uint8_t* image, int numPixels, int firstElt, int lastElt, int splitElt, int splitDist, int treeNode, bool buildForDither, GifPalette* pal)
{
...
// changed pixels only.
int GitRaw::GifPickChangedPixels( const uint8_t* lastFrame, uint8_t* frame, int numPixels )
{
    int numChanged = 0;
...
void GitRaw::GifMakePalette( const uint8_t* lastFrame, const uint8_t* nextFrame, uint32_t width, uint32_t height, int bitDepth, bool buildForDither, GifPalette* pPal )
{
...
void GitRaw::GifDitherImage( const uint8_t* lastFrame, const uint8_t* nextFrame, uint8_t* outFrame, uint32_t width, uint32_t height, GifPalette* pPal )
{
    int numPixels = (int)(width * height);
...
void GitRaw::GifThresholdImage( const uint8_t* lastFrame, const uint8_t* nextFrame, uint8_t* outFrame, uint32_t width, uint32_t height, GifPalette* pPal )
{
...
// insert a single bit
void GitRaw::GifWriteBit( GifBitStatus& stat, uint32_t bit )
{
    bit = bit & 1;
...
void GitRaw::GifWriteChunk( FILE* f, GifBitStatus& stat )
{
...
void GitRaw::GifWriteCode( FILE* f, GifBitStatus& stat, uint32_t code, uint32_t length )
{
...
void GitRaw::GifWritePalette( const GifPalette* pPal, FILE* f )
{
...
void GitRaw::GifWriteLzwImage(FILE* f, uint8_t* image, uint32_t left, uint32_t top,  uint32_t width, uint32_t height, uint32_t delay, GifPalette* pPal)
{
...
bool GitRaw::GifBegin( GifWriter* writer, const char* filename, uint32_t width, uint32_t height, uint32_t delay, int32_t bitDepth , bool dither  )
{
...
bool GitRaw::GifWriteFrame( GifWriter* writer, const uint8_t* image, uint32_t width, uint32_t height, uint32_t delay, int bitDepth , bool dither  )
{
    if(!writer->f) return false;
...
bool GitRaw::GifEnd( GifWriter* writer )
{
    if(!writer->f) return false;
...
void GitRaw::begin(const QString &file, int width, int height, int delay)
{
    GifBegin(&mGifWriter, file.toUtf8().data(), static_cast<quint32>(width), static_cast<quint32>(height), static_cast<quint32>(delay));
}
void GitRaw::frame(const QImage &img, int width, int height, int delay)
{
